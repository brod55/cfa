...
#        docker run --rm -e "GOPATH=/data/" -v ${SERF_PATH}:/data ${GO_VERSION} go build -o /data/bin/${OUTPUT} /data/src/${file_in_src}

        docker run --rm ${DOCKER_RUN_ARGS} -e "CGO_ENABLED=0" -v ${SERF_PATH}/.gopath/src/golang.org:/data/src/golang.org  -v ${SERF_PATH}/.gopath/src/github.com:/data/src/github.com -e "GOPATH=/data/" -v ${SERF_PATH}/../../glusterfs-lib/rwogluster:/data/src/rwogluster   -v ${SERF_PATH}/src/helpers:/data/src/helpers -v ${SERF_PATH}/src/member-update-x:/data/src/memberupdatex -v ${SERF_PATH}/bin:/data/bin -v ${SERF_PATH}/src:/data/serf ${GO_VERSION} go build -a -installsuffix cgo -o /data/bin/${OUTPUT} /data/serf/${file_in_src}
