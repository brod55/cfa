...
if [ -f /etc/docker/daemon.json.orig ]; then
	...
	echo -e "      ,
      \"labels\": [\"system_type=${SYSTEM_TYPE}\", \"vt_enabled=${VT_ENABLED}\", \"hypervisor=${HYPERVISOR}\", \"manufacturer=${MANUFACTURER}\", \"product_name=${PRODUCT_NAME}\", \"serial_number=${SERIAL_NUMBER}\", \"sku_number=${SKU_NUMBER}\", \"family=${FAMILY}\", ${GRAPHICS}],
